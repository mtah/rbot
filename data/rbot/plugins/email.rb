...
class EmailPlugin < Plugin
	...
	def initialize
		...
		@bot.timer.add(@announcement_interval) {
			@announcements.each do |account_name, targets|
				mails = fetch_mail(account_name)
				
				mails.each do |mail|
					#announce the mail to each target
					targets.each {|target| @bot.say(target, "MAIL: '#{mail[:subject]}' from #{mail[:from]}")}
				end
	...
	def delete_announcements(m, params)
		if @announcements.has_key?(params[:account_name])
			...
			m.reply("I am not announcing #{params[:account_name]} anywhere.")
		end
	end
	...
	def check_mail(m, params)	
		if @accounts.has_key?(params[:account_name])
			mails = fetch_mail(params[:account_name])
			
			if mails.size > 0
				mails.each do |mail|
					m.reply "'#{mail[:subject]}' from #{mail[:from]}"
				end
			else
				m.reply("No new mail for account #{params[:account_name]}.")
			end
		else
			m.reply("No account by that name.")
		end
	...
	# fetch and format new emails
	def fetch_mail(account_name)
		mails = []
		account = @accounts[account_name]
		
		pop = Net::POP3.start(account[:server], account[:port], account[:username], account[:password])
		
		# get all mails not yet fetched and process them
		pop.mails.reject {|mail| mail_fetched?(mail)}.each do |mail|
			header = mail.header.split("\r\n")
			subject = rfc2047_decode_to('utf-8', header.grep(/^Subject: /).to_s.split(/^Subject: /)[1])
			from = rfc2047_decode_to('utf-8', header.grep(/^From: /).to_s.split(/^From: /)[1])
